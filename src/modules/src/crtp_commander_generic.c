...
static void fullStateDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  ...
  setpoint->attitude.roll=millirad2deg *values->roll;
  setpoint->attitude.pitch=millirad2deg *values->pitch;
  setpoint->attitude.yaw=millirad2deg *values->yaw;
...
//use this as the flip package
 struct attitudeThrustPack_s {
   uint16_t thrust;
 } __attribute__((packed));
static void attitudeThrustDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  const struct attitudeThrustPack_s *values = data;
  setpoint->mode.roll=modeAbs;
  setpoint->mode.pitch=modeAbs;
  setpoint->mode.yaw=modeAbs;
  setpoint->thrust=values->thrust;
...
const static packetDecoder_t packetDecoders[] = {
  ...
  [attitudeThrustType] =  attitudeThrustDecoder,
};
